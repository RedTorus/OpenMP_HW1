...
unsigned int count_triangles() {
  ...
  // Assign extra iterations to the first 'remainder' threads
  int start = id * blocksize + (id < remainder ? id : remainder);
  int end = start + blocksize + (id < remainder ? 1 : 0);

  for(unsigned int i = start; i < end; ++i) {					

    unsigned int *curr_row_x = IA+i;
    unsigned int *curr_row_A = IA+i+1;
    unsigned int num_nnz_curr_row_x = *curr_row_A - *curr_row_x; //the number of non-zero elements in the current row
    ...
    #pragma omp parallel shared(curr_row_A, y_col_begin, x_col_begin, num_nnz_x, col_x_max, y_col_first, x_col_first) reduction(+:delta)
    {

    int inner_id = omp_get_thread_num();
    int inner_p = omp_get_num_threads();

    int inner_blocksize = num_nnz_y / inner_p;
    int inner_remainder = num_nnz_y % inner_p;

    // Assign extra iterations to first 'inner_remainder' threads
    int inner_start = inner_id * inner_blocksize + (inner_id < inner_remainder ? inner_id : inner_remainder);
    int inner_end = inner_start + inner_blocksize + (inner_id < inner_remainder ? 1 : 0);

    for(unsigned int j = inner_start; j < inner_end; ++j) {

      unsigned int *y_col_local = y_col_begin + j;
      unsigned int row_index_A = *y_col_local - y_col_first; 			
